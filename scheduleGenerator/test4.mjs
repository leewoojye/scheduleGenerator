...
const employeeNames = regulars.map((employee) => employee.name); // 직원 이름 배열 생성

// 기본 설정
const numEmployees = employeeNames.length; // 직원 수
...
function generateInitialPopulation(popSize) {
  ...
  for (let p = 0; p < popSize; p++) {
    ...
    // 직원 배정 (연속 근무 제한을 고려)
    const assignedShifts = {}; // 직원의 근무 횟수 기록
    employeeNames.forEach((name) => (assignedShifts[name] = 0));

    for (let shift = 0; shift < numShifts; shift++) {
      const availableEmployees = employeeNames.filter(
        (name) =>
          assignedShifts[name] < maxShiftsPerEmployee &&
          (shift === 0 || !individual[shift - 1].includes(name)) // 연속 근무 방지
      );
...
function mutate(individual) {
  for (let shift = 0; shift < numShifts; shift++) {
    if (Math.random() < mutationRate) {
      ...
      const availableEmployees = employeeNames.filter(
        (name) =>
          !individual[shift].includes(name) &&
          (shift === 0 || !individual[shift - 1].includes(name)) &&
          (shift === numShifts - 1 || !individual[shift + 1].includes(name))
      );
...
function runGeneticAlgorithm(popSize) {
  ...
  for (let gen = 0; gen < generations; gen++) {
    ...
    console.log(
      `Generation ${
        gen + 1
      }: Best Fitness = ${bestFitness} Best Individual = ${JSON.stringify(
        bestIndividual
      )}`
    );
  }
